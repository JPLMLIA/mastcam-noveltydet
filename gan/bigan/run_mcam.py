...
def create_logdir(method, weight, label, rd):
    ...
    return "./bigan_results/train_logs/mcam/{}/{}/{}/{}".format(weight, method, label, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    ...
    """
    logger = logging.getLogger("BiGAN.train.mcam.{}.{}".format(method,label))

    # Placeholders
    input_pl = tf.placeholder(tf.float32, shape=data.get_shape_input(), name="input")
    tf.summary.histogram('inputs', input_pl, ['image'])
    ...
    testx, testy, testnames = data.get_test('none')
    ...
    with tf.name_scope('losses'):
        ...
        # loss_reconstruction = tf.reduce_mean(tf.norm(tf.contrib.layers.flatten(input_pl-reconstruct), ord=2))
        # loss_features = tf.reduce_mean(tf.norm(tf.contrib.layers.flatten(inter_layer_inp-inter_layer_rct), ord=2))
        ...
        #loss_dis = loss_dis_gen_fake + loss_dis_gen_real
        loss_dis = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.ones_like(l_generator),logits=l_generator))
        loss_generator = 1*loss_dis #+ 0*loss_reconstruction + 0*loss_features

        # encoder
        loss_encoder = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.zeros_like(l_encoder),logits=l_encoder)) #+ 0*loss_reconstruction + 0*loss_features
    ...
    with tf.name_scope('summary'):
        ...
        with tf.name_scope('image_summary'):
            recon_rgb = tf.gather(reconstruct, [2,0,1], axis=3)
            tf.summary.image('reconstruct', recon_rgb, 3, ['image'])
            input_rgb = tf.gather(input_pl, [2,0,1], axis=3)
            tf.summary.image('input_images', input_rgb, 3, ['image'])
            gen_rgb = tf.gather(x_gen, [2,0,1], axis=3)
            tf.summary.image('generated', gen_rgb, 3, ['image'])
    ...
    with sv.managed_session(config=config) as sess:
        ...
        writer = tf.summary.FileWriter(os.path.join(logdir, 'train'), sess.graph)
        val_writer = tf.summary.FileWriter(os.path.join(logdir, 'validation'), sess.graph)
        train_batch = 0
        epoch = 0
        # max_auc = 0
        # max_auc_epoch = 0
        ...
        while not sv.should_stop() and epoch < nb_epochs:
            ...
            # val_loss_dis, val_loss_gen, val_loss_enc = [0, 0, 0]
            ...
            for t in range(nr_batches_train):
                ...
                if t % FREQ_PRINT == 0:  # inspect reconstruction
                    ...
                    writer.add_summary(sm, train_batch)
                    # show validation examples too
                    inds = rng.permutation(testx.shape[0])
                    testx = testx[inds]  # keep shuffling dataset so we see new images
                    testy = testy[inds] # keep shuffling dataset so we see new images
                    sm = sess.run(sum_op_im, feed_dict={input_pl: testx[:batch_size],is_training_pl: False})
                    val_writer.add_summary(sm, train_batch)
            ...
            print("Epoch %d | time = %ds | loss gen = %.4f | loss enc = %.4f | loss dis = %.4f | avg anomaly score = %.4f"
                  % (epoch, time.time() - begin, train_loss_gen, train_loss_enc, train_loss_dis, avg_score))

            epoch += 1
            
        # print("Max AUC = %.4f at Epoch %d" % (max_auc, max_auc_epoch))
        ...
        avg_score = np.mean(scores)
        print("Testing : avg anomaly score = %.4f" % (avg_score))

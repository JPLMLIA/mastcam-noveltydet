...
import importlib
import sys
import gan.mcam_utilities as network
...
def create_logdir(method, weight, label, rd):
    ...
    return "gan/train_logs/mcam/{}/{}/{}/{}".format(weight, method, label, rd)
...
def train_and_test(nb_epochs, weight, method, degree, random_seed, label):
    ...
    """
    logger = logging.getLogger("GAN.train.mcam.{}.{}".format(method,label))
    ...
    # Augment images
    #input_pl = tf.map_fn(lambda frame: tf.image.random_brightness(frame, max_delta=10), input_pl)

    tf.summary.histogram('input_hist', input_pl, ['image'])
    ...
    with tf.name_scope('loss_functions'):
        ...
        real_discriminator_loss = tf.losses.sigmoid_cross_entropy(tf.random_uniform(shape=[batch_size], minval=0.9, maxval=1.0), real_d, scope='real_discriminator_loss')
        #real_discriminator_loss = tf.losses.sigmoid_cross_entropy(tf.constant(0.9, shape=[batch_size]), real_d, scope='real_discriminator_loss')
    ...
    with tf.name_scope('optimizers'):
        ...
        #optimizer_dis = tf.train.GradientDescentOptimizer(learning_rate=0.0002, name='dis_optimizer')
    ...
    with tf.name_scope('training_summary'):
        ...
        with tf.name_scope('image_summary'):
            gen_rgb = tf.gather(generator, [2,0,1], axis=3)
            tf.summary.image('reconstruct', gen_rgb, 3, ['image'])
            input_rgb = tf.gather(input_pl, [2,0,1], axis=3)
            tf.summary.image('input_images', input_rgb, 3, ['image'])
            # tf.summary.image('reconstruct', generator, 3, ['image'])
            # tf.summary.image('input_images', input_pl, 3, ['image'])
    ...
    with tf.variable_scope('Discriminator_loss'):
        if method == "cross-e":
            dis_score = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(fake_d_ema), logits=fake_d_ema)
    ...
    with sv.managed_session() as sess:
        ...
        for t in range(nr_batches_test):
            ...
            for step in range(STEPS_NUMBER):
                ...
                # if step % 20 == 0:
                #     print sess.run(list_scores, feed_dict=feed_dict).tolist()
        ...
        # need to have same batch size at test as in train?
        # fills the rest of the batch with zeros
        ...
        roc_auc = do_roc(scores, testy,
               file_name=r'gan/mcam/{}/{}/{}'.format(method, weight,
                                                     label),
               directory=r'results/gan/mcam/{}/{}/'.format(method,
                                                           weight))
